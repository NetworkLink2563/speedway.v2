...
function Menu_Vms($PrjCode){
    $dbm=new DatabaseManage();
        ...
        WHERE  (dbo.TMstMVmsDirect.XBVmsIsActive = 1) AND (dbo.TMstMVmsDirect.XVTYPE = 1) AND (dbo.TMstMSetupPoint.XVPrjCode = '$PrjCode')
...
function MediaProject($PrjCode){
    ...
    WHERE (XVPrjCode = '$PrjCode') ORDER BY XVMediaVmsCode";
     $result=$dbm->QueryDB($sql);
     $JsonObj = json_decode($result); 
     $CountRec=0;  
     $data="[";
