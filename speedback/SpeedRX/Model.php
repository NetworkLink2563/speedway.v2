...
function Menu_Sub($PrjCode){
    $dbm=new DatabaseManage();
      $sql="SELECT TOP (100) PERCENT dbo.TMstMSpeedEnforce.XVSpeCode, dbo.TMstMSpeedEnforce.XVSpeName, dbo.TMstMSetupPoint.XVPrjCode
      FROM     dbo.TMstMSpeedEnforce INNER JOIN
                        dbo.TMstMSetupPoint ON dbo.TMstMSpeedEnforce.XVSupCode = dbo.TMstMSetupPoint.XVSupCode
...
function ShowCharCountBydate($XVSpeedCode,$datepicker){
    $dbm=new DatabaseManage();
   
    $sql="SELECT 
          [XVSpeedCode]
          ,COUNT([XVSpeedCode]) AS TCountCar
          ,CONVERT(varchar,[XTTime], 23) AS Ttime
          ,DATEPART(hour, [XTTime]) AS Thour
          ,AVG([XISpeed]) AS AvgSpeed				  
           FROM [NWL_VMSControl].[dbo].[TTrnTSpeedEn]
           WHERE CONVERT(varchar,[XTTime], 23)='$datepicker' and XVSpeedCode='$XVSpeedCode'
           GROUP BY  CONVERT(varchar,[XTTime], 23), DATEPART(hour, [XTTime]) , XVSpeedCode
           ORDER BY DATEPART(hour, [XTTime])";
